...
class Instagram():
    ...
    def get_authorization_url():
        get_vars = {
            'app_id': Instagram._app_id,
            'redirect_uri': Instagram._redirect_url,
            'scope': 'user_profile,user_media',
            'response_type': 'code'
        }

        return Instagram._api_base_url + \
            '/oauth/authorize?' + \
            urlencode(get_vars)
    ...
    def get_short_lived_token(instagram_code):
        post_params = {
            'client_id': Instagram._app_id,
            'redirect_uri': Instagram._redirect_url,
            'client_secret': Instagram._app_secret,
            'grant_type': 'authorization_code',
            'code': instagram_code,
        }

        response = requests.post(
            Instagram._api_base_url + "/oauth/access_token",
            post_params
        )
    ...
    def get_long_lived_token(instagram_code):
        short_lived_token, user_id = \
            Instagram.get_short_lived_token(instagram_code)
        ...
        response = requests.get(
            Instagram._graph_base_url + "/access_token",
            get_params
        )
    ...
    def get_latest_image(instagram_code):
        access_token, user_id, _ = \
            Instagram.get_long_lived_token(instagram_code)
        ...
        response = requests.get(
            Instagram._graph_base_url + "/me/media",
            params=get_params
        )
