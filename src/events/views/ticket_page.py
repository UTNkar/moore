...
from swish_integration.models import Payment
...
def my_ticket(request, event_pk):
    ...
    context = {}
    try:
        ticket = Ticket.objects.filter(owner=request.user.id, event=event_pk)[0]
        ...
        try:
            payment = Payment.objects.get(ticket=ticket, user=user)
        except:
            payment = None
        ...
        if not ticket.locked and request.method == "POST":
            if "add_participant" in request.POST and not max_num_participant:
                ...
                queryset = Participant.objects.filter(ticket=ticket).order_by('id')
                formset = ParticipantFormset(queryset=queryset, initial=queryset, form_kwargs={'price_list': event.price_list, "locked": ticket.locked})
            ...
            if formset.is_valid():
                for index, form in enumerate(formset):
                    if form.is_valid():
                        ...
                        # Check for 0 here so that no one tries to delete the first participant via a rogue POST request
                ...
                queryset = Participant.objects.filter(ticket=ticket).order_by('id')
                formset = ParticipantFormset(queryset=queryset, initial=queryset, form_kwargs={'price_list': event.price_list, "locked": ticket.locked})
                ticket.save() # This makes sure that the tickets post_save is called, so that the first user can't be tampered with.
        else:
            formset = ParticipantFormset(queryset=queryset, form_kwargs={'price_list': event.price_list, "locked": ticket.locked})

        # Set this as the first user has to be the owner.
        formset[0].fields['person_nr'].disabled = True
        cost = event.base_price + sum([ participant.calculate_order_cost() for participant in queryset ])
        ...
        context['can_still_pay'] = event.last_payment_date > tz.now() if event.last_payment_date else False
    except ValueError:
        pass
